...
def parse_filename(filename: str) -> Tuple[str, str]:
    try:
        bank, card = filename.split('_')
        return bank, card.replace('.csv', '')
    except ValueError:
        raise ValueError(f"Invalid filename format: {filename}. Expected: bankname_cardname.csv")
...
def transform_to_parquet(input_path: Path) -> Path:
    ...
    bank, card = parse_filename(input_path.name)
    df = pd.read_csv(input_path)
    df = clean_transaction_data(df)
    
    df['credit_card_name'] = card
    df['issuing_bank'] = bank
    
    output_cols = [
        'transaction_date', 'description', 'category', 'tags',
        'amount', 'credit_card_name', 'issuing_bank'
    ]
    df = df[output_cols]
